/**
 * Agenda mensal do departamento (espelho de ministério para testes): calendário e escalas.
 */
...
import { getMinistryByKey } from '../../services/ministries';
import {
  SCHEDULE_TYPES,
...
export default function DepartmentAgendaScreen() {
  const theme = useAppTheme();
  const navigation = useNavigation<any>();
  const route = useRoute<any>();
  const ministryKey = (route.params?.ministryKey ?? 'guest_fire') as string;
  const title = (route.params as any)?.ministryName ?? getMinistryLabel(ministryKey);
  ...
  const [departmentDefaultScheduleType, setDepartmentDefaultScheduleType] = useState<ScheduleTypeId | null>(null);
  ...
  useEffect(() => {
    const validTypes: ScheduleTypeId[] = ['culto', 'midia', 'oracao', 'organizacao', 'financas', 'consolidacao', 'lideranca'];
    getMinistryByKey(ministryKey)
      .then((m) => {
        const t = m?.default_schedule_type;
        setDepartmentDefaultScheduleType(t && validTypes.includes(t as ScheduleTypeId) ? (t as ScheduleTypeId) : null);
      })
      .catch(() => setDepartmentDefaultScheduleType(null));
  }, [ministryKey]);
  ...
  const openScheduleModal = async (ev: AgendaEvent) => {
    ...
    try {
      ...
      if (!type && departmentDefaultScheduleType) {
        type = departmentDefaultScheduleType;
      }
      ...
      setScheduleType((ev.schedule_type as ScheduleTypeId) ?? departmentDefaultScheduleType ?? null);
      setScheduleItems({});
    }
  };
